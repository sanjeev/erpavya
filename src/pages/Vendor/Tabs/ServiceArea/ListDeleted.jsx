...
const ListDeleted = () => {
  ...
  const backUrl = replaceStr(CONFIG.service_area.page.list, ":id", id);
  
  useEffect(() => {

    async function getDetail() {
      const getUrl = replaceStr(CONFIG.service_area.api.deleted, ":id", id);
  ...
  const restoreRecord = async (recordId) => {
    const restoreUrl = replaceStr(CONFIG.service_area.api.restore, ':id', recordId);
  ...
  return (
    <>
      <Breadcrumb pageName={CONFIG.vendor.module} elements={CONFIG.breadcrumbs.service_area.deleted} />
